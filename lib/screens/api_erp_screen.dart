...
class _ApiErpScreenState extends State<ApiErpScreen>
  ...
  Widget _buildStatusBanner() {
    final connected = _erps.where((e) => e.status == ErpStatus.connected).length;
    return Container(
  ...
  Widget _statusChip(ErpStatus status) {
    final (color, label, icon) = switch (status) {
      ...
      ErpStatus.disconnected => (AppColors.textLabel, 'Inactivo', Icons.circle_outlined),
        ...
        Container(
          ...
          child: Column(
            children: [
              ...
              const Divider(height: 1),
              _infoRow(Icons.calendar_today_outlined, 'Creada',
                  '15 Mar 2026'),
              const Divider(height: 1),
              _infoRow(Icons.schedule_outlined, 'Expira', 'Nunca'),
              const Divider(height: 1),
              _infoRow(Icons.checklist_rounded, 'Permisos',
                  'Lectura / Escritura'),
              const Divider(height: 1),
              _infoRow(Icons.bolt_rounded, 'Rate limit',
                  '1,000 req / min'),
            ],
          ),
        ),
      ...
      children: [
        ...
        Container(
          ...
          child: Column(
            children: [
              _syncLogRow(
                  '✅', 'SAP Business One', 'Exportación CT-48293', 'Hace 12 min', true),
              const Divider(height: 1),
              _syncLogRow(
                  '✅', 'SAP Business One', 'Actualización aranceles', 'Hace 1 h', true),
              const Divider(height: 1),
              _syncLogRow(
                  '❌', 'Oracle NetSuite', 'Timeout de conexión', 'Hace 3 h', false),
              const Divider(height: 1),
              _syncLogRow(
                  '✅', 'SAP Business One', 'Importación CT-92104', 'Hace 5 h', true),
            ],
          ),
        ),
        const SizedBox(height: 20),
  ...
  Widget _syncLogRow(String emoji, String system, String event,
      String time, bool success) {
    return Padding(
  ...
  void _showDisconnectDialog(ErpSystem erp) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: Text('Desconectar ${erp.name}',
