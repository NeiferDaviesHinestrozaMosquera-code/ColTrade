...
class _ChecklistScreenState extends State<ChecklistScreen>
  ...
  final _docs = [
    _DocItem('Factura Comercial', 'DIAN', true, false),
    _DocItem('Lista de Empaque (Packing List)', 'Empresa', true, false),
    _DocItem('Certificado de Origen', 'MINCIT', false, true),
    _DocItem('Declaración de Exportación (DEX)', 'DIAN', false, false),
    _DocItem('Fitosanitario ICA', 'ICA', false, false, hasError: true),
  ];
  ...
  Widget _buildHeader() {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text('COLTRADE AI',
              style: AppTextStyles.labelUppercase.copyWith(color: const Color(0xFF94A3B8))),
          ...
          Row(
            children: [
              ...
              Text('Destino: Puerto de Rotterdam, NL',
                  style: AppTextStyles.caption.copyWith(color: const Color(0xFFCBD5E1))),
            ],
          ),
  ...
  Widget _buildDocumentsTab() {
    final completed = _docs.where((d) => d.completed).length;
    return ListView(
        ...
        Row(
          ...
          children: [
            ...
            Container(
              ...
              child: Text('$completed/${_docs.length}',
                  style: AppTextStyles.badgeText.copyWith(color: AppColors.textSecondary)),
            ),
          ],
        ),
        const SizedBox(height: 12),

        ..._docs.map((doc) => _buildDocRow(doc)),
  ...
  Widget _buildDocRow(_DocItem doc) {
    return Container(
        ...
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: doc.completed
                  ? null
                  : Border.all(
                      color: doc.hasError ? AppColors.errorRed : AppColors.border,
                      width: 1.5),
          ...
          if (doc.needsUpload)
            Container(
              ...
              child: Text('SUBIR',
                  style: AppTextStyles.badgeText.copyWith(fontSize: 10, color: Colors.white)),
            )
          else
  ...
  Widget _buildOrderTab() {
    final steps = [
      (TimelineStatus.completed, 'Solicitud creada', 'Inicio del proceso', '12 Oct, 09:00'),
      (TimelineStatus.completed, 'Documentos DIAN listos', 'Factura y lista de empaque aprobados', '14 Oct, 11:30'),
      (TimelineStatus.active, 'Obtener Certificado ICA', 'Pendiente de inspección fitosanitaria', null),
  ...
  Widget _buildEntitiesTab() {
    final entities = [
      ('DIAN', 'Dirección de Impuestos y Aduanas Nacionales', Icons.account_balance_rounded,
          AppColors.primaryDarkNavy, true),
      ('ICA', 'Instituto Colombiano Agropecuario', Icons.eco_rounded, AppColors.successGreen, false),
      ('INVIMA', 'Instituto Nacional de Vigilancia', Icons.health_and_safety_outlined,
          AppColors.infoBlue, false),
      ('MinCIT', 'Ministerio de Comercio, Industria y Turismo', Icons.business_center_outlined,
          AppColors.accentOrange, false),
      ('VUCE', 'Ventanilla Única de Comercio Exterior', Icons.computer_outlined,
          Colors.teal, false),
    ];
...
class _DocItem {
  ...
  _DocItem(this.name, this.entity, this.completed, this.needsUpload, {this.hasError = false});
}
